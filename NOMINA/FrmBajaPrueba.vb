...
Public Class FrmBajaPrueba
        ...
        If fila > 0 Then
            ...
            Try
                ...
                If cs.consultas(scr, datos, msj) Then
                    scr = "SELECT * FROM TB_LIQUIDACION_PRUEBA WHERE cod_empleado = " & codigo & " and cod_empresa = " & empresa
                    If cs.consultas(scr, dtliquid, msj) = False Then
        ...
        Else
            ...
            If resulta = MsgBoxResult.Yes Then
                Try
                    ...
                    scr_insert = "INSERT INTO TB_LIQUIDACION_PRUEBA (COD_EMPLEADO,COD_EMPRESA,FEC_LIQUIDACION,MOTIVO)VALUES (" & codigo & "," & empresa & ",'" & Format(DatFecha.Value, f) & "','" & Cbtipo.SelectedItem & "')"
                    scr_vacas = "EXECUTE VACAS_BAJA_PRUEBA @fecha = '" & Format(DatFecha.Value, f) & "', @empresa = " & empresa & ", @cod_emp =" & codigo
                    scr_aguin = "EXECUTE AGUINALDO_BAJA_PRUEBA @FECHA ='" & Format(DatFecha.Value, f) & "',@EMPRESA = " & empresa & " , @COD_EMP = " & codigo
                    scr_bono = "EXECUTE BONO14_BAJA_PRUEBA @fecha = '" & Format(DatFecha.Value, f) & "', @empresa = " & empresa & ", @cod_emp =" & codigo
                    scr_anticpo = "EXECUTE ANTICIPOS_BAJA_PRUEBA @fecha = '" & Format(DatFecha.Value, f) & "', @empresa = " & empresa & ", @codigo =" & codigo
                    scr_indemni = "INDEMNIZACION_PRUEBA @fecha = '" & Format(DatFecha.Value, f) & "', @empresa = " & empresa & ", @cod_emp =" & codigo
                    scr_mat = "MATERIALES_BAJA_P @fecha = '" & Format(DatFecha.Value, f) & "', @empresa = " & empresa & ", @codigo =" & codigo
                    ...
                    If Cbtipo.SelectedItem = "REORGANIZACION" Then
                        ind = "S"
                    End If
                    If cs.baja_prueba(scr_insert, scr_vacas, scr_aguin, scr_bono, scr_anticpo, scr_indemni, scr_mat, ind, msj) Then
                        muestra_totales()
                        llena_fg()
                    End If
                Catch ex As Exception
    ...
    Private Sub TxtNombre_TextChanged(sender As System.Object, e As System.EventArgs) Handles TxtNombre.TextChanged
        scr = "select codigo,nombre, segundo_nombre, apellido,segundo_apellido, fec_alta, fec_baja from tb_empleado " & _
                "where   nombre like  '%" & TxtNombre.Text & "%'  and codigo_empresa =" & empresa & "  and estado_empleado in ('A','S') "
    ...
    Private Sub Txt2nombre_TextChanged(sender As System.Object, e As System.EventArgs) Handles Txt2nombre.TextChanged
        scr = "select codigo,nombre, segundo_nombre, apellido,segundo_apellido, fec_alta, fec_baja from tb_empleado " & _
              "where   apellido like  '%" & Txt2nombre.Text & "%'  and codigo_empresa =" & empresa & "  and estado_empleado in ('A','S') "
        If cs.consultas(scr, dt, msj) Then
            llena_fg()
        End If
    End Sub
    ...
    Private Sub TxCodigo_TextChanged(sender As System.Object, e As System.EventArgs) Handles TxCodigo.TextChanged
        scr = "select codigo,nombre, segundo_nombre, apellido,segundo_apellido, fec_alta, fec_baja from tb_empleado " & _
              "where   codigo like  '%" & TxCodigo.Text & "%'  and codigo_empresa =" & empresa & "  and estado_empleado in ('A','S') "
        If cs.consultas(scr, dt, msj) Then
            llena_fg()
        End If
    End Sub
    ...
    Private Sub BtImprimir_Click(sender As System.Object, e As System.EventArgs) Handles BtImprimir.Click
        ...
        Dim strsql As String = " SELECT A.*,B.NOMBRE,B.SEGUNDO_NOMBRE,B.APELLIDO,B.SEGUNDO_APELLIDO,FEC_ALTA,FEC_BAJA,C.NOMBRE_PUESTO,(MTO_AGUINALDO+MTO_BONO14+MTO_INDEMNIZACION+MTO_VACACIONES-MTO_ANTICIPOS)AS MONTO FROM TB_LIQUIDACION_PRUEBA AS A" & _
        ...
        Try
            ...
            Dim texto As TextField = DirectCast(rp.Fields("nombre"), TextField)
            texto.Text = nombre
            Dim cantiLetras As TextField = DirectCast(rp.Fields("cantiletras"), TextField)
            cantiLetras.Text = canti
        ...
        Try
            ...
            pdffilter.FileName = pathPDF
            rp.RenderToFilter(pdffilter)
    ...
    Private Sub FrmBaja_CLOSE(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.FormClosing
        scr = "DELETE FROM TB_LIQUIDACION_PRUEBA"
