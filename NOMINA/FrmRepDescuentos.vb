...
Public Class FrmRepDescuentos
    ...
    Private Sub guarda_pdf()
        ...
        Try
            rp.Load("C:\Plan_rep\nomina.flxr", "descuentos")
            rp.DataSource.ConnectionString = cs.con
            rp.DataSource.RecordSource = "SELECT a.*, B.NOMBRE,B.SEGUNDO_NOMBRE,B.APELLIDO,B.SEGUNDO_APELLIDO,C.NOMBRE AS DEPART,D.DESCRIPCION, '" & f & "' as f FROM TB_DESCUENTO AS A " &
                                            " LEFT JOIN TB_EMPLEADO AS B ON A.CODIGO = B.CODIGO" &
                                            " LEFt JOIN TB_DEPTOS_EMPRESA AS C ON b.COD_DEPART = c.COD_DEPAR" &
                                            " LEFT JOIN TB_TIPODESC AS D ON A.COD_DESC = d.CODIGO" &
